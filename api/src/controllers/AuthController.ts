...
class AuthController {
    ...
    public sendConfirmation = async (req: Request, res: Response, next: NextFunction) => {
        try {
            const { email } = req.body;
            await this.loginService.sendEmailConfirmation(email);
            return res.json(true);
        } catch (err) {
            console.log(err);
            next(err);
    ...
    public confirmEmail = async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const validated = await this.userService.confirmEmail(user_token);
            if (validated == true)
