...
class LoginService {
    ...
    public async doUserLogin (username: string, password: string): Promise<string> {
        ...
        const newToken = jwt.sign({ id: user.id, name: user.name, email: user.email, email_validated: user.email_validated}, '123', {
            expiresIn: 3600
        });
        return newToken;
    }
    ...
    public sendEmailConfirmation = async (email: string): Promise<boolean> => {
        const user = await this.userRepository.getUserByEmail(email);
        if (!user) throw new NotFoundException('Usuário não encontrado');
        return await this.queueService.sendEmailConfirmationToQueue({user_token: user.user_token, email: user.email});
    }
}
