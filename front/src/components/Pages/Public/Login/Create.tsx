...
const LoginForm: React.FC<IProps> = ({ onCancel, className }) => {
  const form = useForm<IUser>({
    ...
    async onSubmit(model) {
      ...
      try {
        await authService.sendEmailConfirmation(model.email, model.user_token);
        Toast.info('Foi enviado um link para seu email para podermos validar o endereço e confirmar seu acesso.');
        //form.reset();
      } catch (err) {
        Toast.error('Não foi possível enviar o email de validação.');
      }
    }
  });
